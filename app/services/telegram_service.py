...
async def tg_edit_message(chat_id: Optional[int], message_id: Optional[int], text: str,
    ...
    Возвращает True, если:
      ...
      - текст не изменился (message is not modified) — считаем, что для UI это «успех».
    ...
    # В Telegram editMessageText допускает ЛИБО (chat_id, message_id), ЛИБО inline_message_id
    ...
    if "message is not modified" in desc:
        # Ничего страшного — просто не изменилось
        return True
    if "message can't be edited" in desc or "message to edit not found" in desc:
        # Нельзя редактировать — вызывающий пусть решит, что делать (обычно sendMessage)
        return False

    # неизвестная ошибка — отдаём False
    return False
...
async def tg_edit_or_send(chat_id: int, message_id: Optional[int], text: str,
    ...
    Возвращает JSON ответа последней удачной операции.
    """
    ok = await tg_edit_message(chat_id, message_id, text, reply_markup=reply_markup)
    if ok:
        # для единообразия возвращаем «успех» без деталей
        return {"ok": True}
